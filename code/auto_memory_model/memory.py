...
class DynamicMemory(nn.Module):
    ...
    def predict_everything(self, ment_idx, mem_vectors, last_ment_vectors,
        ...
        coref_new_prob = torch.nn.functional.log_softmax(
            torch.cat([coref_score, torch.tensor([0.0]).cuda()]), dim=0)
        coref_logits = coref_new_prob[:self.num_cells]
        new_ent_logit = coref_new_prob[self.num_cells]
        ...
        # Combine the scores
        overwrite_score = new_ent_logit + torch.nn.functional.log_softmax(fert_score, dim=0)

        comb_score = torch.cat([coref_logits, overwrite_score], dim=0)
        # Multiplicative mask
