...
mlp_size_list = ['-mlp_size ' + str(mlp_size) for mlp_size in [1000, 2000]]
dropout_list = ['-dropout_rate ' + str(dropout_rate) for dropout_rate in [0.2, 0.3, 0.5]]
...
entity_rep_list = ['-entity_rep ' + str(entity_rep) for entity_rep in ['max', 'avg', 'lstm', 'gru']]
lr_list = ['-init_lr ' + str(lr) for lr in [5e-4]]
seed = ['-seed 0']

JOB_NAME = 'auto_mem'

out_dir = path.join(os.getcwd(), 'outputs/ent_repr')
