...
mlp_size_list = ['-mlp_size ' + str(mlp_size) for mlp_size in [1000, 1500, 3000]]
dropout_list = ['-dropout_rate ' + str(dropout_rate) for dropout_rate in [0.2, 0.3, 0.4]]
...
ment_emb_list = ['-ment_emb ' + str(ment_emb) for ment_emb in ['attn', 'endpoint']]
lr_list = ['-init_lr ' + str(lr) for lr in [5e-4]]
seed = ['-seed 0']

JOB_NAME = 'auto_mem'

out_dir = path.join(os.getcwd(), 'outputs/simple')
...
with open(out_file, 'w') as out_f:
    for option_comb in product(mlp_size_list, dropout_list, num_cell_list,
