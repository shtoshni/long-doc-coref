...
mem_type_list = ['-mem_type unbounded -sample_ignores 0.2 ', '-mem_type unbounded -sample_ignores 1.0 ']
dataset_list = ['-dataset ontonotes -no_singletons -max_epochs 5 ']
doc_enc_list = ['-doc_enc ' + mem_type for mem_type in ['overlap']]
model_size_list = ['-model_size ' + str(model_size) for model_size in ['large']]
...
num_cell_list = ['-num_cells {} '.format(x) for (x) in [25]]
...
out_dir = path.join(os.getcwd(), 'outputs/ontonotes_exp_1')
...
common_options = [dataset_list, doc_enc_list, model_size_list,
                  ...
                  max_segment_list, lr_list, seed,
                  over_loss_wt_list, new_ent_wt_list]
