...
cross_val_split_list = ['-cross_val_split ' + str(cross_val_split) for cross_val_split in range(1, 6)]
num_train_docs_list = ['-num_train_docs ' + str(num_train_docs) for num_train_docs in [10, 20, 40, 80]]
...
num_cell_list = ['-num_cells {} -new_ent_wt {}'.format(x, y) for (x, y) in [(20, 5.0)]]
over_loss_wt_list = ['-over_loss_wt ' + str(over_loss_wt) for over_loss_wt in [0.1]]

JOB_NAME = 'sparse_sup'

out_dir = path.join(os.getcwd(), 'outputs/sparsity')
...
common_options = [doc_enc_list, mlp_size_list, mlp_depth_list, coref_mlp_depth_list,
                  num_train_docs_list, dropout_list, model_loc_list,
