...
mem_type_list = ['-mem_type unbounded -ignore_wt 0.2 ', '-mem_type fixed_mem -ignore_wt 0.1 ',
                 '-mem_type lru -ignore_wt 0.2 ']
dataset_list = ['-dataset litbank ']
doc_enc_list = ['-doc_enc ' + mem_type for mem_type in ['overlap']]
model_size_list = ['-model_size ' + str(model_size) for model_size in [MODEL_SIZE]]
...
num_cell_list = ['-num_cells {} '.format(x) for (x) in [5, 10, 20, 50]]
train_span_model_list = ['-train_span_model -top_span_ratio 0.3 ']

JOB_NAME = f'cross_val_litbank_{MODEL_SIZE}'

out_dir = path.join(os.getcwd(), f'outputs/cross_val_litbank_{MODEL_SIZE}')
...
common_options = [dataset_list, doc_enc_list, mlp_size_list, mlp_depth_list, coref_mlp_depth_list,
