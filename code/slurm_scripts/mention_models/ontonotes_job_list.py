...
doc_enc_list = ['-doc_enc ' + mem_type for mem_type in ['independent', 'overlap']]
...
max_span_width_list = ['-max_span_width ' + str(max_span_width) for max_span_width in [30]]
max_segment_list = ['-max_segment_len ' + str(max_segment_len) for max_segment_len in [384, 512]]
max_epochs_list = ['-max_epochs 10']
dataset_list = ['-dataset ontonotes']
lr_list = ['-init_lr ' + str(lr) for lr in [5e-4]]
seed = ['-seed 0']

JOB_NAME = 'ontonotes_ment'

out_dir = path.join(os.getcwd(), 'outputs/ontonotes')
...
common_options = [mlp_size_list, mlp_depth_list, doc_enc_list,
                  dropout_list, pretrained_bert_list, model_size_list,
                  max_span_width_list, max_segment_list, max_epochs_list,
                  dataset_list, lr_list, seed]
