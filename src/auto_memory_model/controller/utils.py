...
def pick_controller(mem_type='unbounded', dataset='litbank', device='cuda', **kwargs):
    ...
    #     model = ControllerIgnore(dataset=dataset, device=device, **kwargs).to(device)
    if mem_type in ['learned', 'unbounded', 'lru']:
        model = ControllerPredInvalid(mem_type=mem_type, dataset=dataset, device=device, **kwargs).to(device)
        ...
        model = ControllerNoInvalid(dataset=dataset, device=device, **kwargs).to(device)
    else:
