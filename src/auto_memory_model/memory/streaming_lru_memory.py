...
class StreamingLRUMemory(BaseFixedMemory):
    def __init__(self, **kwargs):
        super(StreamingLRUMemory, self).__init__(**kwargs)

    def initialize_memory(self, mem=None, ent_counter=None, last_mention_idx=None, lru_list=None):
        ...
        return mem, ent_counter, last_mention_idx, lru_list
    ...
    def forward(self, mention_emb_list, mention_scores, gt_actions, metadata, rand_fl_list,
                memory_init=None, last_action_str='<s>', ment_offset=0,
                teacher_forcing=False):
        # Initialize memory
        # Initialize memory
        if memory_init:
            mem_vectors, ent_counter, last_mention_idx, lru_list = self.initialize_memory(**memory_init)
        else:
            mem_vectors, ent_counter, last_mention_idx, lru_list = self.initialize_memory()
        ...
        ment_idx = ment_offset

        for idx, (ment_emb, ment_score, (gt_cell_idx, gt_action_str)) in \
                enumerate(zip(mention_emb_list, mention_scores, gt_actions)):
            ment_idx = ment_offset + idx
            ...
            if not (follow_gt and gt_action_str == 'i' and rand_fl_list[idx] > self.sample_invalid):
        ...
        return (coref_new_list, new_ignore_list, action_list,
                {"mem": mem_vectors, "ent_counter": ent_counter,
                 "last_mention_idx": last_mention_idx, "lru_list": lru_list}, last_action_str, ment_idx)
