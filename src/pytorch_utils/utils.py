...
def get_sequence_mask(sequence_len):
    ...
    tmp = torch.arange(max_len).expand(batch_size, max_len).to(sequence_len.device)
    return tmp < sequence_len.unsqueeze(1)


def get_span_mask(start_ids, end_ids, max_len):
    tmp = torch.arange(max_len).unsqueeze(0).expand(start_ids.shape[0], -1).to(start_ids.device)
    batch_start_ids = start_ids.unsqueeze(1).expand_as(tmp).to(start_ids.device)
    batch_end_ids = end_ids.unsqueeze(1).expand_as(tmp).to(start_ids.device)
